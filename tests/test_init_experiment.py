...
def test_scaffold_uses_project_named_dataset_and_trainer(tmp_path: Path) -> None:
    ...
    assert (target_dir / "src" / "criterions" / "__init__.py").exists()
    assert (target_dir / "src" / "optimizers" / "__init__.py").exists()
    assert (target_dir / "src" / "schedulers" / "__init__.py").exists()
